...
class KBZipje(filename: String)
{
   ...
   lazy val documentStream:Stream[() => Node] =  zipUtils.findPath(root).filter(p => p.toString.endsWith(".xml")).map(p => () => XML.load(Files.newInputStream(p)))
   ...
   lazy val textQuery = ListDisjunction(queryTerms.toList.map(SingleTerm(_)))
   //println(queryTerms)
